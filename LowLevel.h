...
public:
    ...
    bool Astar(doubleVertex* start, doubleVertex* goal, Path &path,const vector<Constraint*> &constraints,int agentMaxSpeed){
        ...
        while (!open.empty()&&closed.size() < m.getVertexNum() * 30){
            int index= get_node_with_least_f(open);
                ...
                for(int i=0;i<leftSuccessors.size();i++){
                    ...
                    if(it==open.end())
                    {
                        ...
                        if(!hasConflict(leftSuccessors[i],current->depth+step,constraints))
                        {
                            ...
                            leftSuccessors[i]->dvParent->visitLeft=true;
                ...
                for(int i=0;i<rightSuccessors.size();i++){
                    ...
                    if(it==open.end())
                    {
                        ...
                        if(!hasConflict(rightSuccessors[i],current->depth+step,constraints))
                        {
                            ...
                            rightSuccessors[i]->dvParent->visitRight=true;
    ...
    int get_node_with_least_f(const std::vector<doubleVertex*> &list) const
    {
        //TODO
        double min = INT_MAX;
        int min_index = 0;
